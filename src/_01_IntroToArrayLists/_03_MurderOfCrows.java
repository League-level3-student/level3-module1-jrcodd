	...
	private void findTheDiamond() {
		...
		for (int i = 0; i < theMurder.size(); i++) {
			if (theMurder.get(i).getStomachContents().contains("diamond")) {
				System.out.print(theMurder.get(i).getName());
				System.out.println(" had " + theMurder.get(i).getStomachContents());
				JOptionPane.showMessageDialog(null, i
						+ " Innocent crows died and got grusomly cut open. Did you know that when you cut open a cat it can still meow with the air in its chest???");
			}
		}
	}
